...
def get_inference_model(model_config_path):
    ...
    cnn_model = tf.keras.applications.EfficientNetB0(
        input_shape=(*model_config["IMAGE_SIZE"], 3), include_top=False, weights="imagenet"
    )
    cnn_model.trainable = False
    cnn_model_out = cnn_model.output
    cnn_model_out = tf.keras.layers.Reshape((-1, 1280))(cnn_model_out)
    cnn_model = tf.keras.models.Model(cnn_model.input, cnn_model_out)

    encoder = tf.keras.layers.MultiHeadAttention(
        num_heads=NUM_HEADS, key_dim=EMBED_DIM
    )
    decoder = tf.keras.layers.MultiHeadAttention(
        num_heads=NUM_HEADS, key_dim=EMBED_DIM
    )

    caption_model = tf.keras.Model(cnn_model.input, decoder(encoder(cnn_model_out, cnn_model_out), cnn_model_out))
    caption_model.load_weights("IMC/weights.h5")

    return caption_model

model = get_inference_model("IMC/config_train.json")
...
def generate_caption(image_path, caption_model, tokenizer, SEQ_LENGTH):
    ...
    img = tf.image.decode_jpeg(img, channels=3)
    img = tf.image.resize(img, model_config["IMAGE_SIZE"])
    img = tf.image.convert_image_dtype(img, tf.float32)
    img = tf.expand_dims(img, axis=0)
...
if uploaded_file is not None:
    ...
    # Generate caption
    caption = generate_caption("temp.jpg", model, tokenizer, model_config["SEQ_LENGTH"])
